...
from drifting.models.model import DriftDiT_Tiny, DriftDiT_Small, DriftDiT_models
from drifting.models.drifting import (
    compute_V,
    normalize_features,
    normalize_drift,
)
from drifting.models.feature_encoder import create_feature_encoder, pretrain_mae
...
def _mnist_exists(root: Path) -> bool:
    """Check whether MNIST files are already present."""
...
def _cifar10_exists(root: Path) -> bool:
    """Check whether CIFAR-10 files are already present."""
...
def get_dataset(name: str, root: str = "data") -> tuple:
    """Get dataset and transforms with minimal re-downloading."""
...
def train(
    ...
    # Resolve data root (default to repo-level data folder)
    ...
    # Load dataset
    train_dataset, test_dataset = get_dataset(dataset, root=str(data_root))
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Root directory for datasets (defaults to repo data folder)",
    )
    parser.add_argument(
        "--resume",
        type=str,
